...
## 11.3 标注即输入：宿主采集，小程序别自己写

图片类小程序有一半代码在干同一件事：**让用户在图上圈一下**。拖框、八个缩放把手、缩放换算、边界钳制 —— imagefix 的界面 250 行里有 200 行是这个，idcard 的四点标注又写了一遍。

这件事**不该由小程序做**，理由有三个，一个比一个硬：

1. **重复。** 每个图片小程序都重写一遍，而且一遍比一遍糙。
2. **坐标换算是最容易错的地方**，而它错了不会崩，只会悄悄框歪。
3. **AI 写 GUI 最容易翻车的正是这部分。** 业务逻辑（"裁剪→送模型→回贴"）AI 写得很好，拖拽手柄和 DPI 换算它写不稳。把这部分从小程序里拿掉，"AI 写小程序"的门槛才真的降下来。

所以小程序**声明**它需要什么标注，宿主负责采集：

```jsonc
"ui": {
  "annotation": {
    "kind": "rect",                  // rect | quad | point | freehand
    "target_field": "region",        // 采集结果塞进动作的哪个入参
...
宿主打开自己的查看/标注面，采集完**换算成源图自然像素**，塞进 `target_field`，再调动作。声明了 `annotation` 的小程序不需要写任何选取界面。

### 坐标契约

宿主侧的标注通常用**分数坐标（0~1）**存储 —— 这样跨窗口大小、跨 DPI、跨会话重开都对得上。而动作契约要求**源图自然像素**（§12：无头调用时根本没有"预览"这个概念）。

两者都对，是同一问题的两半。换算发生在宿主，只有一次：

```
自然像素 = 分数坐标 × unit.renderSize
```

诚实的边界：

- `renderSize` 对**图片**就是自然尺寸，换算干净。
- 对**分页/矢量格式**（PDF 页、CAD），`renderSize` 是该格式自己的坐标系而不是像素；小程序若要按像素处理，宿主 MUST 先在一个明确的 DPI 下栅格化，并把那个尺寸作为自然像素基准。
- 纯文本这类没有二维画布的格式，`renderSize` 可以缺失 —— 此时宿主 MUST 拒绝 `annotation` 声明，而不是塞一个瞎猜的坐标进去。

### 校验与退路

校验器会核对声明是否自洽：`target_field` / `image_field` 必须存在于该动作的 `input_schema`；形状要对得上（`rect` 产出对象、`quad`/`freehand`/`point` 产出数组）。对不上直接拒装 —— 否则宿主把标注塞进一个不存在的字段，错误要等用户点下去才暴露。

宿主采不了某种 `kind` 时（例如标注面还没有四点采集），MUST 退回小程序自带的界面，而不是报错。**声明是优化，不是依赖** —— 小程序仍应能独立工作。
